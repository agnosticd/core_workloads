...
== Keycloak Additional Features

When using Keycloak as the authentication provider, additional features are available:

=== Additional OAuth Clients

You can configure additional OAuth clients for applications like GitLab, Grafana, etc.
...
- clientId: gitlab
  ...
  redirectUris:
  - "https://gitlab-gitlab.apps.example.com/*"
...
You can create custom groups for admin/cluster accounts and regular user accounts.
Users are automatically assigned to groups based on their account type.
...
# Admin groups - assigned to admin and cluster accounts
...
# User groups - assigned to regular user accounts
...
* Admin and cluster accounts will be assigned to: `/admins`, `/cluster-admins`, `/operations`
* Regular user accounts will be assigned to: `/users`, `/developers`, `/team-leads`
* Applications can use group membership and attributes for access control
